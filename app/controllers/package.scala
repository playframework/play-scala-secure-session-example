...
import play.api.libs.crypto.{CookieSigner, CryptoConfig}
...
package object controllers {
  ...
  class UserInfoAction @Inject() (
    ...
    messagesApi: MessagesApi
      ...
      extends ActionBuilder[UserRequest] with Results {
  ...
  class UserInfoCookieBakerFactory @Inject() (
    ...
    injectedCookieSigner: CookieSigner
   ) {

    def createCookieBaker(secretKey: Array[Byte]): EncryptedCookieBaker[UserInfo] = {
      new EncryptedCookieBaker[UserInfo](secretKey, encryptionService, cryptoConfig) {
        ...
        override def cookieSigner: CookieSigner = injectedCookieSigner
      }
    }
  }
